// time complexity=O(NlogM)       N is the size of array B and log M is due to the power function.
// space complexity=O(N)           	- To store factorials up to  A.
...
int Solution::solve(int A, vector<int> &B) {
    ...
    ans = (ans*fact[x])%mod;
    ans = (ans*power(p,mod-2))%mod;                 //fermats theorm  note mode should me prime  (a^-1)%,mod==a^(mod-2)%mod
    return ans;                                  
}
